...
*/
TB_API UCS4 decode(const char *&src, const char *src_end);
...
*/
TB_API int encode(UCS4 ch, char *dst);
...
*/
TB_API UCS4 decode_next(const char *str, int *i, int i_max);
...
/** Move to the next character in a UTF-8 string.
	...
	@param i_max The last position (size of str).
*/
TB_API void move_inc(const char *str, int *i, int i_max);
...
*/
TB_API void move_dec(const char *str, int *i);
...
/** Count characters before null termination in a UTF-8 string.
	...
	@param i_max The last position (size of str).
*/
TB_API int count_characters(const char *str, int i_max);
