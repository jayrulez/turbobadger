...
TBSkin::TBSkin(TBContext* context)
	...
	, m_default_spacing(0)
{
	m_context->GetRenderer()->AddListener(this);
...
bool TBSkin::LoadInternal(const char *skin_file)
{
	...
	if (node.GetNode("description"))
	{
		...
		if (TBNode *supported_dpi_node = node.GetNode("description>supported-dpi"))
		{
			...
			{
				int screen_dpi = m_context->GetSystemInterface()->GetDPI();
...
TBSkin::~TBSkin()
{
	m_context->GetRenderer()->RemoveListener(this);
}
...
{
	if (!dst_rect.IsEmpty())
		m_context->GetRenderer()->DrawBitmapColored(dst_rect, TBRect(), color, m_color_frag);
}
...
{
	TBRect rect = dst_rect.Expand(element->expand, element->expand);
	m_context->GetRenderer()->DrawBitmapTile(rect, element->bitmap->GetBitmap());
}
...
void TBSkin::PaintElementBitmap(const TBRect &dst_rect, const TBRect &src_rect, TBSkinElement *element)
{
	if (element->img_color != 0)
		m_context->GetRenderer()->DrawBitmapColored(dst_rect, src_rect, element->img_color, element->bitmap);
	else
		m_context->GetRenderer()->DrawBitmap(dst_rect, src_rect, element->bitmap);
}
...
TBSkinElement::TBSkinElement(TBContext *context)
	: skinContext(context)
