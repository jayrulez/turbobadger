...
bool TBBitmapFragmentMap::ValidateBitmap()
{
	if (m_need_update)
	{
		if (m_bitmap)
			...
			m_bitmap = g_tb_context->GetRenderer()->CreateBitmap(m_bitmap_w, m_bitmap_h, m_bitmap_data);
		m_need_update = false;
	}
...
void TBBitmapFragmentManager::FreeFragment(TBBitmapFragment *frag)
{
	if (frag)
	{
		g_tb_context->GetRenderer()->FlushBitmapFragment(frag);
...
void TBBitmapFragmentManager::Debug()
{
	...
	for (int i = 0; i < m_fragment_maps.GetNumItems(); i++)
	{
		...
		if (TBBitmap *bitmap = fm->GetBitmap())
			g_tb_context->GetRenderer()->DrawBitmap(TBRect(x, 0, fm->m_bitmap_w, fm->m_bitmap_h), TBRect(0, 0, fm->m_bitmap_w, fm->m_bitmap_h), bitmap);
		x += fm->m_bitmap_w + 5;
	}
}
#endif // TB_RUNTIME_DEBUG_INFO
