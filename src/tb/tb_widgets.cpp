...
public:
	...
	{
		PostMessageDelayed(TBIDC("TBLongClickTimer"), nullptr, get_system_interface()->GetLongClickDelayMS());
	}
...
TBWidget::PaintProps::PaintProps()
{
	...
	text_color = get_tb_skin()->GetDefaultTextColor();
}
...
TBWidget::~TBWidget()
{
	...
	if (this == get_context()->hovered_widget)
		get_context()->hovered_widget = nullptr;
	if (this == get_context()->captured_widget)
		get_context()->captured_widget = nullptr;
	if (this == get_context()->focused_widget)
		get_context()->focused_widget = nullptr;
...
{
	get_context()->update_widget_states = true;
	InvalidateSkinStates();
}

void TBWidget::InvalidateSkinStates()
{
	get_context()->update_skin_states = true;
}
...
WIDGET_STATE TBWidget::GetAutoState() const
{
	...
	bool add_pressed_state = !get_context()->cancel_click && this == get_context()->captured_widget && this == get_context()->hovered_widget;
	if (add_pressed_state)
		state |= WIDGET_STATE_PRESSED;
	if (this == get_context()->hovered_widget && (!m_packed.no_automatic_hover_state || add_pressed_state))
		state |= WIDGET_STATE_HOVERED;
	if (this == get_context()->focused_widget && get_context()->show_focus_state)
...
{
	if (get_context()->show_focus_state == on)
		return;
	get_context()->show_focus_state = on;
	if (get_context()->focused_widget)
		get_context()->focused_widget->Invalidate();
}
...
void TBWidget::RemoveChild(TBWidget *child, WIDGET_INVOKE_INFO info)
{
	...
	if (info == WIDGET_INVOKE_INFO_NORMAL)
	{
		...
		if (!m_packed.is_dying && child == get_context()->focused_widget)
...
TBSkinElement *TBWidget::GetSkinBgElement()
{
	...
	return get_tb_skin()->GetSkinElementStrongOverride(m_skin_bg, static_cast<SKIN_STATE>(state), context);
}
...
{
	if (get_context()->focused_widget == this)
		return true;
	...
	{
		get_context()->focused_widget->Invalidate();
		get_context()->focused_widget->InvalidateSkinStates();
	}

	TBWidgetSafePointer old_focus(get_context()->focused_widget);
	get_context()->focused_widget = this;
	...
	if (info == WIDGET_INVOKE_INFO_NORMAL)
	{
		...
		if (get_context()->focused_widget && get_context()->focused_widget == this)
			get_context()->focused_widget->OnFocusChanged(true);
		if (get_context()->focused_widget && get_context()->focused_widget == this)
			TBWidgetListener::InvokeWidgetFocusChanged(get_context()->focused_widget, true);
	}
	return true;
}
...
{
	TBWidget *origin = get_context()->focused_widget;
	if (!origin)
		origin = this;
...
void TBWidget::OnPaintChildren(const PaintProps &paint_props)
{
	...
	int child_translation_x, child_translation_y;
	GetChildTranslation(child_translation_x, child_translation_y);
	get_renderer()->Translate(child_translation_x, child_translation_y);

	TBRect clip_rect = get_renderer()->GetClipRect();
	...
	for (TBWidget *child = GetFirstChild(); child; child = child->GetNext())
	{
		if (clip_rect.Intersects(child->m_rect) && child->GetVisibility() == WIDGET_VISIBILITY_VISIBLE)
		{
			...
			if (skin_element && skin_element->HasOverlayElements())
			{
				...
				float old_opacity = get_renderer()->GetOpacity();
				float opacity = old_opacity * child->CalculateOpacityInternal(state, skin_element);
				if (opacity > 0)
				{
					get_renderer()->SetOpacity(opacity);

					TBWidgetSkinConditionContext context(child);
					get_tb_skin()->PaintSkinOverlay(child->m_rect, skin_element, static_cast<SKIN_STATE>(state), context);

					get_renderer()->SetOpacity(old_opacity);
				}
			}
		}
	}
	...
	if (get_context()->focused_widget && get_context()->focused_widget->m_parent == this)
	{
		TBWidgetSkinConditionContext context(get_context()->focused_widget);
		TBSkinElement *skin_element = get_context()->focused_widget->GetSkinBgElement();
		if (!skin_element || !skin_element->HasState(SKIN_STATE_FOCUSED, context))
		{
			WIDGET_STATE state = get_context()->focused_widget->GetAutoState();
			if (state & SKIN_STATE_FOCUSED)
				get_tb_skin()->PaintSkin(get_context()->focused_widget->m_rect, TBIDC("generic_focus"), static_cast<SKIN_STATE>(state), context);
		}
	}
...
PreferredSize TBWidget::GetPreferredSize(const SizeConstraints &in_constraints)
{
	...
	TB_IF_DEBUG_SETTING(LAYOUT_PS_DEBUGGING, last_measure_time = get_system_interface()->GetTimeMS());
...
{
	if (!get_context()->update_skin_states && !force_update)
		return;
	get_context()->update_skin_states = false;
...
{
	if (!get_context()->update_widget_states && !force_update)
		return;
	get_context()->update_widget_states = false;
...
float TBWidget::CalculateOpacityInternal(WIDGET_STATE state, TBSkinElement *skin_element) const
{
	...
	if (state & WIDGET_STATE_DISABLED)
		opacity *= get_tb_skin()->GetDefaultDisabledOpacity();
...
void TBWidget::InvokePaint(const PaintProps &parent_paint_props)
{
	...
	get_renderer()->Translate(trns_x, trns_y);

	// Paint background skin
	TBRect local_rect(0, 0, m_rect.w, m_rect.h);
	TBWidgetSkinConditionContext context(this);
	TBSkinElement *used_element = get_tb_skin()->PaintSkin(local_rect, skin_element, static_cast<SKIN_STATE>(state), context);
	assert(!!used_element == !!skin_element);

	TB_IF_DEBUG_SETTING(LAYOUT_BOUNDS, get_tb_skin()->PaintRect(local_rect, TBColor(255, 255, 255, 50), 1));
	...
	if (used_element)
		get_renderer()->Translate(used_element->content_ofs_x, used_element->content_ofs_y);
	...
	if (TB_DEBUG_SETTING(LAYOUT_PS_DEBUGGING))
	{
		...
		const double now = get_system_interface()->GetTimeMS();
		if (now < last_layout_time + debug_time)
		{
			get_tb_skin()->PaintRect(local_rect, TBColor(255, 30, 30, 200), 1);
			Invalidate();
		}
		if (now < last_measure_time + debug_time)
		{
			get_tb_skin()->PaintRect(local_rect.Shrink(1, 1), TBColor(255, 255, 30, 200), 1);
			Invalidate();
		}
	}
#endif // TB_RUNTIME_DEBUG_INFO
...
{
	if (!get_context()->captured_widget)
	{
		SetCapturedWidget(GetWidgetAt(x, y, true));
		SetHoveredWidget(get_context()->captured_widget, touch);
		//captured_button = button;

		// Hide focus when we use the pointer, if it's not on the focused widget.
		if (get_context()->focused_widget != get_context()->captured_widget)
			SetAutoFocusState(false);

		// Start long click timer. Only for touch events for now.
		if (touch && get_context()->captured_widget && get_context()->captured_widget->GetWantLongClick())
			get_context()->captured_widget->StartLongClickTimer(touch);

		// Get the closest parent window and bring it to the top
		TBWindow *window = get_context()->captured_widget ? get_context()->captured_widget->GetParentWindow() : nullptr;
		if (window)
			window->Activate();
	}
	if (get_context()->captured_widget)
	{
		...
		TBWidget *tmp = get_context()->captured_widget;
		while (tmp)
		{
		...
		TBWidget *focus_target = get_context()->captured_widget;
		while (focus_target)
		{
	...
	if (get_context()->captured_widget)
	{
		get_context()->captured_widget->ConvertFromRoot(x, y);
		get_context()->pointer_move_widget_x = get_context()->pointer_down_widget_x = x;
		get_context()->pointer_move_widget_y = get_context()->pointer_down_widget_y = y;
		TBWidgetEvent ev(EVENT_TYPE_POINTER_DOWN, x, y, touch, modifierkeys);
		ev.count = click_count;
		get_context()->captured_widget->InvokeEvent(ev);
...
bool TBWidget::InvokePointerUp(int x, int y, MODIFIER_KEYS modifierkeys, bool touch)
{
	if (get_context()->captured_widget)
	{
		...
		get_context()->captured_widget->InvokeEvent(ev_up);
		if (!get_context()->cancel_click && get_context()->captured_widget && get_context()->captured_widget->GetHitStatus(x, y))
			get_context()->captured_widget->InvokeEvent(ev_click);
		if (get_context()->captured_widget) // && button == captured_button
			get_context()->captured_widget->ReleaseCapture();
...
{
	StopLongClickTimer();
	if (get_context()->captured_widget == this &&
		!get_context()->cancel_click &&
		get_context()->captured_widget->GetHitStatus(get_context()->pointer_move_widget_x, get_context()->pointer_move_widget_y))
	{
		// Invoke long click
		TBWidgetEvent ev_long_click(EVENT_TYPE_LONG_CLICK, get_context()->pointer_move_widget_x, get_context()->pointer_move_widget_y, touch, TB_MODIFIER_NONE);
		bool handled = get_context()->captured_widget->InvokeEvent(ev_long_click);
		if (!handled)
		{
			// Long click not handled so invoke a context menu event instead
			TBWidgetEvent ev_context_menu(EVENT_TYPE_CONTEXT_MENU, get_context()->pointer_move_widget_x, get_context()->pointer_move_widget_y, touch, TB_MODIFIER_NONE);
			handled = get_context()->captured_widget->InvokeEvent(ev_context_menu);
		}
...
void TBWidget::InvokePointerMove(int x, int y, MODIFIER_KEYS modifierkeys, bool touch)
{
	...
	TBWidget *target = get_context()->captured_widget ? get_context()->captured_widget : get_context()->hovered_widget;
	if (target)
	{
		target->ConvertFromRoot(x, y);
		get_context()->pointer_move_widget_x = x;
		get_context()->pointer_move_widget_y = y;
...
void TBWidget::HandlePanningOnMove(int x, int y)
{
	...
	const int dx = get_context()->pointer_down_widget_x - x;
	const int dy = get_context()->pointer_down_widget_y - y;
	const int threshold = get_system_interface()->GetPanThreshold();
	...
	if (get_context()->captured_widget->m_packed.is_panning || maybe_start_panning_x || maybe_start_panning_y)
	{
		// The threshold is met for not invoking any long click
		get_context()->captured_widget->StopLongClickTimer();

		int start_compensation_x = 0, start_compensation_y = 0;
		if (!get_context()->captured_widget->m_packed.is_panning)
		{
		...
		TBScroller *scroller = get_context()->captured_widget->GetReadyScroller(dx != 0, dy != 0);
		if (!scroller)
			return;

		int old_translation_x = 0, old_translation_y = 0;
		get_context()->captured_widget->GetScrollRoot()->GetChildTranslation(old_translation_x, old_translation_y);

		if (scroller->OnPan(dx + start_compensation_x, dy + start_compensation_y))
		{
			// Scroll delta changed, so we are now panning!
			get_context()->captured_widget->m_packed.is_panning = true;
			get_context()->cancel_click = true;
			...
			get_context()->captured_widget->GetScrollRoot()->GetChildTranslation(new_translation_x, new_translation_y);
			get_context()->pointer_down_widget_x += new_translation_x - old_translation_x + start_compensation_x;
			get_context()->pointer_down_widget_y += new_translation_y - old_translation_y + start_compensation_y;
		}
	}
}
...
bool TBWidget::InvokeKey(int key, SPECIAL_KEY special_key, MODIFIER_KEYS modifierkeys, bool down)
{
	...
	if (get_context()->focused_widget)
	{
		...
		if (!modifierkeys && get_context()->focused_widget->GetClickByKey() &&
			!get_context()->focused_widget->GetDisabled() &&
			!get_context()->focused_widget->GetIsDying() &&
			(special_key == TB_KEY_ENTER || key == ' '))
		{
			...
			{
				had_pressed_state = get_context()->focused_widget->GetState(WIDGET_STATE_PRESSED);
			...
			{
				get_context()->focused_widget->SetState(WIDGET_STATE_PRESSED, down);
				get_context()->focused_widget->m_packed.has_key_pressed_state = down;
			}
		...
		else
		{
			...
			handled = get_context()->focused_widget->InvokeEvent(ev);
		}
	}
...
{
	if (get_context()->hovered_widget == widget)
		return;
	...
	{
		get_context()->hovered_widget->Invalidate();
		get_context()->hovered_widget->InvalidateSkinStates();
	}

	get_context()->hovered_widget = widget;
	...
	if (get_context()->hovered_widget)
	{
		...
		get_context()->hovered_widget->m_packed.no_automatic_hover_state = touch;
	}
}
...
{
	if (get_context()->captured_widget == widget)
		return;
	if (widget && widget->GetState(WIDGET_STATE_DISABLED))
		return;
	...
	if (get_context()->captured_widget)
	{
		...
		if (TBScroller *scroller = get_context()->captured_widget->FindStartedScroller())
		{
			if (get_context()->captured_widget->m_packed.is_panning)
		...
		get_context()->captured_widget->m_packed.is_panning = false;

		// We apply pressed state automatically so the widget might need to be updated.
		get_context()->captured_widget->Invalidate();
		get_context()->captured_widget->InvalidateSkinStates();

		get_context()->captured_widget->StopLongClickTimer();
	}
	get_context()->cancel_click = false;

	TBWidget *old_capture = get_context()->captured_widget;

	get_context()->captured_widget = widget;
	...
	if (get_context()->captured_widget)
	{
		get_context()->captured_widget->Invalidate();
		get_context()->captured_widget->InvalidateSkinStates();
		get_context()->captured_widget->OnCaptureChanged(true);
	}
}
...
bool TBWidget::SetFontDescription(const TBFontDescription &font_desc)
{
	...
	if (get_font_manager()->HasFontFace(font_desc))
		m_font_desc = font_desc;
	else if (get_font_manager()->CreateFontFace(font_desc))
		m_font_desc = font_desc;
	else
		return false;
...
TBFontDescription TBWidget::GetCalculatedFontDescription() const
{
	...
	return get_font_manager()->GetDefaultFontDescription();
}

TBFontFace *TBWidget::GetFont() const
{
	return get_font_manager()->GetFontFace(GetCalculatedFontDescription());
}
