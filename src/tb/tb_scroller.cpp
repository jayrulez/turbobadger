...
bool TBScroller::OnPan(int dx, int dy)
{
	...
	double now_ms = m_target->GetContext()->GetSystemInterface()->GetTimeMS();
	if (m_pan_time_ms)
	{
...
{
	if (m_target->GetContext()->GetSystemInterface()->GetTimeMS() < m_pan_time_ms + PAN_START_THRESHOLD_MS)
	{
		// Don't start scroll if we have too little speed.
		// This will prevent us from scrolling accidently.
		float pan_start_distance_threshold_px = 2 * m_target->GetContext()->GetSystemInterface()->GetDPI() / 100.0f;
...
{
	// Set start values
	m_scroll_start_ms = m_target->GetContext()->GetSystemInterface()->GetTimeMS();
	...
	if (!GetMessageByID(TBIDC("scroll")))
	{
		...
		PublishMessageDelayed(TBIDC("scroll"), nullptr, (uint32)PAN_MSG_DELAY_MS);
	}
}
