...
bool TBScroller::OnPan(int dx, int dy)
{
	...
	double now_ms = get_system_interface()->GetTimeMS();
	if (m_pan_time_ms)
	{
...
{
	if (get_system_interface()->GetTimeMS() < m_pan_time_ms + PAN_START_THRESHOLD_MS)
	{
		// Don't start scroll if we have too little speed.
		// This will prevent us from scrolling accidently.
		float pan_start_distance_threshold_px = 2 * get_system_interface()->GetDPI() / 100.0f;
...
{
	// Set start values
	m_scroll_start_ms = get_system_interface()->GetTimeMS();
