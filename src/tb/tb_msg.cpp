...
{
	return PostMessageOnTime(message, data, g_tb_context->GetSystemInterface()->GetTimeMS() + (double)delay_in_ms);
}
...
bool TBMessageHandler::PostMessageOnTime(TBID message, TBMessageData *data, double fire_time)
{
	if (TBMessage *msg = new TBMessage(message, data, fire_time, this))
	{
		...
		if (!g_all_normal_messages.GetFirst() && g_all_delayed_messages.GetFirst() == msg)
			g_tb_context->GetSystemInterface()->RescheduleTimer(msg->fire_time_ms);
		return true;
	}
	return false;
}
...
bool TBMessageHandler::PublishMessage(TBID message, TBMessageData *data)
{
	if (TBMessage *msg = new TBMessage(message, data, 0, this))
	{
		...
		if (g_all_normal_messages.GetFirst() == msg)
			g_tb_context->GetSystemInterface()->RescheduleTimer(0);
		return true;
	}
	return false;
}
...
void TBMessageHandler::DeleteMessage(TBMessage *msg)
{
	...
	// Note: We could call g_tb_context->GetSystemInterface()->RescheduleTimer if we think that deleting
...
void TBMessageHandler::ProcessMessages()
{
	...
	{
		if (g_tb_context->GetSystemInterface()->GetTimeMS() >= msg->fire_time_ms)
		{
