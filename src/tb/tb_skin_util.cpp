	...
	if (TBSkinElement *skin = context->GetSkin()->GetSkinElement(skin_x))
	{
		if (skin->bitmap)
		{
			...
			if ((dw = GetFadeoutSize(left, bw)) > 0)
				context->GetRenderer()->DrawBitmap(TBRect(dst_rect.x, dst_rect.y, dw, dst_rect.h), TBRect(0, 0, bw, bh), skin->bitmap);
			if ((dw = GetFadeoutSize(right, bw)) > 0)
				context->GetRenderer()->DrawBitmap(TBRect(dst_rect.x + dst_rect.w - dw, dst_rect.y, dw, dst_rect.h), TBRect(bw, 0, -bw, bh), skin->bitmap);
		}
	}
	if (TBSkinElement *skin = context->GetSkin()->GetSkinElement(skin_y))
	{
		if (skin->bitmap)
		{
			...
			if ((dh = GetFadeoutSize(top, bh)) > 0)
				context->GetRenderer()->DrawBitmap(TBRect(dst_rect.x, dst_rect.y, dst_rect.w, dh), TBRect(0, 0, bw, bh), skin->bitmap);
			if ((dh = GetFadeoutSize(bottom, bh)) > 0)
				context->GetRenderer()->DrawBitmap(TBRect(dst_rect.x, dst_rect.y + dst_rect.h - dh, dst_rect.w, dh), TBRect(0, bh, bw, -bh), skin->bitmap);
		}
	}
}
