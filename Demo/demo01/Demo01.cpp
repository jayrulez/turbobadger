...
{
	// We could do g_tb_context->GetWidgetsReader()->LoadFile(this, filename) but we want
...
{
	// We could do g_tb_context->GetWidgetsReader()->LoadData(this, filename) but we want
	// some extra data we store under "WindowInfo", so read into node tree.
	TBNode node;
	node.ReadData(data);
	LoadResource(node);
...
{
	g_tb_context->GetWidgetsReader()->LoadNodeTree(this, &node);
	...
	const TBDimensionConverter *dc = g_tb_context->GetSkin()->GetDimensionConverter();
	...
	virtual bool OnEvent(const TBWidgetEvent &ev)
	{
		if (ev.type == EVENT_TYPE_CLICK)
		{
			...
			else if (ev.target->GetID() == TBIDC("popup_menu"))
			{
				...
				{
					TBFontDescription fd = g_tb_context->GetFontManager()->GetDefaultFontDescription();
					fd.SetSize(28);
...
void MainWindow::OnMessageReceived(TBMessage *msg)
{
	...
	else if (msg->message == TBIDC("delayedmsg"))
	{
		...
		text.SetFormatted("Delayed message received!\n\n"
							...
							(int)(g_tb_context->GetSystemInterface()->GetTimeMS() - msg->GetFireTime()));
...
bool MainWindow::OnEvent(const TBWidgetEvent &ev)
{
	if (ev.type == EVENT_TYPE_CLICK)
	{
		...
		{
			int reload_count = 10;
			double t1 = g_tb_context->GetSystemInterface()->GetTimeMS();
			for (int i = 0; i < reload_count; i++)
				g_tb_context->GetSkin()->ReloadBitmaps();
			double t2 = g_tb_context->GetSystemInterface()->GetTimeMS();
		...
		{
			g_tb_context->GetRenderer()->InvokeContextLost();
			g_tb_context->GetRenderer()->InvokeContextRestored();
			TBMessageWindow *msg_win = new TBMessageWindow(ev.target, TBID());
...
void DemoApplication::RenderFrame()
{
	...
	//g_tb_context->GetImageManager()->Debug();
#endif
...
void DemoApplication::OnBackendAttached(AppBackend *backend, int width, int height)
{
	...
	// Load language file
	g_tb_context->GetLanguage()->Load("resources/language/lng_en.tb.txt");
	...
	g_tb_context->GetFontManager()->AddFontInfo("resources/fonty_skin/fontawesome.ttf", "FontAwesome");
	g_tb_context->GetSkin()->Load("resources/fonty_skin/skin.tb.txt", "Demo/demo01/skin/skin.tb.txt");
#else
	g_tb_context->GetSkin()->Load("resources/default_skin/skin.tb.txt", "Demo/demo01/skin/skin.tb.txt");
#endif

	// Add fonts we can use to the font manager.
#if defined(TB_FONT_RENDERER_STB) || defined(TB_FONT_RENDERER_FREETYPE)
	g_tb_context->GetFontManager()->AddFontInfo("Demo/fonts_ttf/Lato-Regular.ttf", "Lato");
#endif
#ifdef TB_FONT_RENDERER_TBBF
	g_tb_context->GetFontManager()->AddFontInfo("resources/default_font/segoe_white_with_shadow.tb.txt", "Segoe");
	g_tb_context->GetFontManager()->AddFontInfo("Demo/fonts/neon.tb.txt", "Neon");
	g_tb_context->GetFontManager()->AddFontInfo("Demo/fonts/orangutang.tb.txt", "Orangutang");
	g_tb_context->GetFontManager()->AddFontInfo("Demo/fonts/orange.tb.txt", "Orange");
#endif
	...
	fd.SetID(TBIDC("Lato"));
	fd.SetSize(g_tb_context->GetSkin()->GetDimensionConverter()->DpToPx(15));
#elif defined(TB_FONT_RENDERER_STB)
	fd.SetID(TBIDC("Lato"));
	fd.SetSize(g_tb_context->GetSkin()->GetDimensionConverter()->DpToPx(18));
#else
	fd.SetID(TBIDC("Segoe"));
	fd.SetSize(g_tb_context->GetSkin()->GetDimensionConverter()->DpToPx(14));
#endif
	g_tb_context->GetFontManager()->SetDefaultFontDescription(fd);

	// Create the font now.
	TBFontFace *font = g_tb_context->GetFontManager()->CreateFontFace(g_tb_context->GetFontManager()->GetDefaultFontDescription());
