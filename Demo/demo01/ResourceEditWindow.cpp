	...
	, m_source_edit(nullptr)
{
	...
	GetContext()->GetWidgetsReader()->LoadFile(this, "Demo/demo01/ui_resources/resource_edit_window.tb.txt");
...
void ResourceEditWindow::RefreshFromSource()
{
	...
	GetContext()->GetWidgetsReader()->LoadData(m_build_container, m_source_edit->GetText());
...
bool ResourceEditWindow::OnEvent(const TBWidgetEvent &ev)
{
	...
	else if (ev.type == EVENT_TYPE_CLICK && ev.target->GetID() == TBIDC("test"))
	{
		...
		if (TBWindow *win = new TBWindow())
		{
			...
			GetContext()->GetWidgetsReader()->LoadData(win->GetContentRoot(), m_source_edit->GetText());
...
void ResourceEditWindow::OnPaintChildren(const PaintProps &paint_props)
{
	...
	if (TBWidget *selected_widget = GetSelectedWidget())
	{
		...
		selected_widget->GetContext()->ConvertToRoot(widget_rect.x, widget_rect.y);
		ConvertFromRoot(widget_rect.x, widget_rect.y);
		GetContext()->GetSkin()->PaintRect(widget_rect, TBColor(255, 205, 0), 1);
	}
}
